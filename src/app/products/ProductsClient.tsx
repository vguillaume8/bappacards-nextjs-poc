...
export default function ProductsClient() {
  ...
  const handleDesignNow = (productId: string) => {
    window.location.href = `https://designer.bappacards.com/?product=${productId}`;
  };
  ...
  return (
    <>
      ...
      >
        <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 2 }}>
          <Typography
            variant="h1"
            component="h1"
            sx={{ fontWeight: 700, mb: 3, fontSize: { xs: '2.75rem', md: '4rem' }, letterSpacing: '-0.02em' }}
          >
          ...
          <Typography
            variant="h6"
            component="p"
            sx={{ fontWeight: 400, mb: 6, maxWidth: '760px', mx: 'auto', lineHeight: 1.6, color: 'rgba(255,255,255,0.9)' }}
          >
            ...
            {[
              ...
              <Box
                ...
                sx={{
                  ...
                  bgcolor: 'rgba(235,28,36,0.15)', border: '1px solid rgba(235,28,36,0.3)',
                  px: 3, py: 1.5, borderRadius: '12px',
                }}
              >
                <Box sx={{ display: 'flex', alignItems: 'center', color: '#EB1C24' }}>{benefit.icon}</Box>
                <Typography sx={{ fontWeight: 600, fontSize: '0.95rem' }}>{benefit.text}</Typography>
              </Box>
            ))}
          </Box>
        </Container>
      ...
      {/* Products Grid */}
      <Box component="section" sx={{ py: { xs: 5, md: 8 }, background: 'linear-gradient(to bottom, #f8f9fa, #f0f2f5)' }}>
        <Container maxWidth="xl">
          <Typography variant="h3" component="h2" sx={{ fontWeight: 600, mb: 2, textAlign: 'center' }}>
            Our Premium Collections
          </Typography>
          <Typography variant="body1" sx={{ textAlign: 'center', color: '#555', mb: 8, maxWidth: '700px', mx: 'auto', fontSize: '1.1rem' }}>
              ...
              <article key={product.id}>
                <Card
                  sx={{
                    ...
                    position: 'relative', border: product.mostPopular ? '3px solid #EB1C24' : 'none',
                  }}
                >
                  {product.mostPopular && (
                    <Box sx={{ position: 'absolute', top: 16, right: 16, zIndex: 3 }}>
                      <Chip label="MOST POPULAR" sx={{ bgcolor: '#EB1C24', color: 'white', fontWeight: 700, fontSize: '0.75rem' }} />
                    </Box>
                  )}
                  <Box
                    sx={{
                      height: { xs: 220, md: 280 }, display: 'flex', alignItems: 'center', justifyContent: 'center',
                      bgcolor: product.mostPopular ? '#1a1a1a' : '#f8f9fa',
                    }}
                  >
                    <Typography sx={{ color: product.mostPopular ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.15)', fontSize: '4rem', fontWeight: 700 }}>
                      {product.topLine.charAt(0)}
                    </Typography>
                  </Box>
                  <CardContent sx={{ flexGrow: 1, p: { xs: 3, md: 4 } }}>
                    <Typography variant="subtitle1" color="primary" sx={{ fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', fontSize: '0.85rem', mb: 1 }}>
                      {product.topLine}
                    </Typography>
                    <Typography variant="h5" component="h3" sx={{ fontWeight: 700, mb: 2, color: '#263238' }}>
                      {product.headline}
                    </Typography>
                    <Typography variant="h3" sx={{ fontWeight: 700, color: '#EB1C24', mb: 2 }}>
                      ${product.price}
                    </Typography>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <Button
                        ...
                        disabled={isLoading}
                        sx={{ bgcolor: '#EB1C24', '&:hover': { bgcolor: '#c2003c', transform: 'translateY(-2px)' }, borderRadius: '12px', py: 1.5, textTransform: 'none', fontWeight: 600, fontSize: '1rem', transition: 'all 0.3s ease' }}
                      >
                        Design Now
                      </Button>
                      <Button
                        ...
                        disabled={isLoading}
                        sx={{ borderColor: '#EB1C24', color: '#EB1C24', borderWidth: '2px', '&:hover': { borderColor: '#c2003c', borderWidth: '2px', backgroundColor: 'rgba(235,28,36,0.08)' }, borderRadius: '12px', py: 1.5, textTransform: 'none', fontWeight: 600, fontSize: '1rem', transition: 'all 0.3s ease' }}
                      >
                        Design Later
                      </Button>
                    </Box>
      ...
      <Box component="section" sx={{ py: { xs: 5, md: 8 }, bgcolor: '#fafafa' }}>
        <Container maxWidth="lg">
          <Typography variant="h3" component="h2" sx={{ textAlign: 'center', fontWeight: 600, mb: 2 }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 3 }}>
            {testimonials.map((t, index) => (
              <article key={index}>
                <Card sx={{ p: 4, height: '100%', boxShadow: '0 4px 12px rgba(0,0,0,0.06)', borderRadius: '16px', border: '1px solid rgba(0,0,0,0.06)', transition: 'all 0.3s ease', '&:hover': { transform: 'translateY(-8px)', boxShadow: '0 8px 24px rgba(0,0,0,0.1)' } }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                    <Avatar sx={{ width: 64, height: 64, border: '3px solid #EB1C24', bgcolor: '#EB1C24', fontWeight: 700, fontSize: '1.5rem' }}>
                      {t.name[0]}
                    </Avatar>
                    <Box>
                      <Typography variant="subtitle1" sx={{ fontWeight: 600, fontSize: '1.1rem' }}>{t.name}</Typography>
                      <Typography variant="body2" color="text.secondary" sx={{ fontWeight: 500 }}>{t.role}</Typography>
                      {t.company && <Typography variant="caption" color="text.secondary">{t.company}</Typography>}
                    </Box>
                  </Box>
                  <Box sx={{ mb: 2 }}>
                    {[...Array(5)].map((_, i) => (
      ...
      <Box component="section" sx={{ py: { xs: 5, md: 8 }, bgcolor: 'white' }}>
        <Container maxWidth="md">
          ...
          {faqs.map((faq, index) => (
            <Accordion key={index} sx={{ mb: 2, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', borderRadius: '12px !important', '&:before': { display: 'none' } }}>
      ...
      <Snackbar open={Boolean(errorMsg)} autoHideDuration={6000} onClose={() => setErrorMsg(null)} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
        <Alert onClose={() => setErrorMsg(null)} severity="error" sx={{ width: '100%', borderRadius: '12px' }}>{errorMsg}</Alert>
      </Snackbar>
      <Snackbar open={Boolean(successMsg)} autoHideDuration={6000} onClose={() => setSuccessMsg(null)} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
        <Alert onClose={() => setSuccessMsg(null)} severity="success" sx={{ width: '100%', borderRadius: '12px' }}>{successMsg}</Alert>
      </Snackbar>
    </>
  );
}
